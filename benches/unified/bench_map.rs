...
#[inline(never)]
fn bench_get2<T: Map<u64>>(bencher: Bencher<'_, '_>) {
  let mut t: [_; 10] =
    SIZES.map(|n| {
      let mut t = T::new();
      for i in 0 .. n { t.insert(make_key(i), i as u64); }
      t
    });
  ...
  bencher.bench_local(|| {
    ...
    'done: loop {
      for t in t.iter_mut() {
        for i in 0 .. 500 {
          match t.get(make_key(i)) { None => (), Some(&y) => { k += y; } };
          n = n - 1;
          if n == 0 { break 'done; }
        }
      }
