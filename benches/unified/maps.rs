...
pub(crate) trait Map<T> {
  ...
  fn insert(&mut self, _: NonZeroU64, _: T) -> Option<T>;
...
impl<T> Map<T> for tangerine::map::IntMap<NonZeroU64, T> {
  #[inline(always)]
  ...
  #[inline(always)]
  fn insert(&mut self, k: NonZeroU64, v: T) -> Option<T> { self.insert(k, v) }

  #[inline(always)]
  fn remove(&mut self, k: NonZeroU64) -> Option<T> { self.remove(k) }

  fn for_each_value<F: FnMut(&T)>(&self, f: F) { self.values().for_each(f) }
}

impl<T, S: Default + std::hash::BuildHasher> Map<T> for std::collections::HashMap<NonZeroU64, T, S> {
  #[inline(always)]
